...
| `veh_trace_basic_blocks` | `threadId, start, end, ..., collect_events?, max_events?, collect_memory_writes?, collect_memory_reads?, dependency_sources?, start_condition?, stop_condition?, collect_condition?` | Bounded trace with versioned aggregate metadata, an optional ordered block-transition event stream, block/edge coverage, memory observations, dependencies, summaries, regions, and exceptions. |
