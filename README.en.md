...
| `veh_trace_basic_blocks` | `threadId, start, end, ..., collect_events?, collect_memory_events?, max_memory_events?, collect_code?, ...` | Bounded trace with versioned aggregate metadata, optional ordered block/code/memory-occurrence streams, block/edge coverage, memory observations, dependencies, summaries, regions, and exceptions. |
