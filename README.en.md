...
The trace IPC negotiates request/header sizes and recognizes the preceding schema-v3/v4 wire layouts, preserving array offsets when a running MCP server and a newly injected DLL straddle a deployment. When the current collector rejects startup, `failure` reports status, a specific reason, the stopped-state decision, normalized IP/start/end, decode success, and the decoded-instruction count.
...
`collect_code=true` also enables ordered events and retains runtime block bytes as unique `(block, version)` records. Event-schema-v2 `code_version` fields link occurrences to `code_versions`, preserving which bytes were observed at each sequence even for self-modifying code. `max_code_bytes` (up to 16 MiB) and `max_code_versions` are independent total budgets; exhaustion keeps aggregate tracing active while setting `code_truncated=true` and `code_capture.complete=false`. The default byte budget remains 256 KiB, so only large VM traces need to opt into the larger allocation.
