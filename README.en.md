...
| `veh_trace_basic_blocks` | `threadId, start, end, ..., collect_events?, collect_code?, max_code_bytes?, max_code_versions?, ...` | Bounded trace with versioned aggregate metadata, optional ordered transition/runtime code-version streams, block/edge coverage, memory observations, dependencies, summaries, regions, and exceptions. |
