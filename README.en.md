...
**Discover an unknown protected-code path**
...
Every response declares `schema_version=4`, `mode=aggregated`, and `thread_id`, making the aggregation semantics and thread scope explicit. Aggregate data alone cannot reconstruct execution order. With `collect_events=true`, event-schema-v1 `events` record the initial block entry and every subsequent block transition in execution order, each with its trace-step `sequence` and OS `thread_id`. This is a basic-block-transition stream, not an instruction stream. If `max_events` is exceeded, aggregate collection continues while `events_truncated=true` and `ordering.complete=false` explicitly mark the partial stream.

The trace IPC negotiates request/header sizes and recognizes legacy schema-v3/v4 plus the preceding wire-v5 layout, preserving array offsets when a running MCP server and a newly injected DLL straddle a deployment. When the current collector rejects startup, `failure` reports status, a specific reason, the stopped-state decision, normalized IP/start/end, decode success, and the decoded-instruction count.

`collect_memory_events=true` also enables ordered block events and retains per-occurrence reads and writes in the same trace-step sequence space. Every `memory_events` record carries the thread, instruction, effective address, size, logical `access_index`, read value or write before/after values, and dependency origins. Exceeding the independent `max_memory_events` budget keeps aggregate tracing active while `memory_events_truncated`, the exact `memory_events_dropped` count, and `memory_ordering.complete=false` expose the loss. Kind plus access index distinguishes multiple operands and read-modify-write accesses within one instruction occurrence.

`collect_register_events=true` also enables ordered block events and retains every normally completed instruction occurrence in the active collection window. Each `register_events` record uses the shared sequence and includes the OS thread, instruction address, and before/after values for changed GPRs, SP, and EFLAGS; an empty `changes` object still preserves an unchanged occurrence. Faulting instructions remain in the exception stream rather than receiving a guessed completed delta. Exceeding the independent `max_register_events` budget reports `register_events_truncated`, an exact `register_events_dropped` count, and `register_ordering.complete=false`.

`collect_code=true` also enables ordered events and retains runtime block bytes as unique `(block, version)` records. Event-schema-v2 `code_version` fields link occurrences to version IDs, preserving which bytes were observed at each sequence even for self-modifying code. The default `code_output="inline"` retains the existing `code_versions` JSON shape and allows `max_code_bytes` up to 16 MiB.

Large VM traces can select `code_output="file"`. The VEH path writes only to a preallocated ring while a separate writer sends configurable 256 KiB–8 MiB `code_chunk_bytes` frames (4 MiB default) over a one-shot data pipe to the MCP process. MCP writes a portable little-endian `.vtc` artifact and returns its path, size, SHA-256, and actual byte/version/chunk counts. File mode allows `max_code_bytes` up to 400 MiB, never overwrites an existing destination, and creates a unique host-temp file when `code_output_path` is omitted. Budget or backpressure exhaustion keeps aggregate tracing active while setting `code_truncated=true` and `code_capture.complete=false`. The artifact uses 64-bit offsets so an Analyzer on another OS can consume Windows x86/x64 captures.
...
| `veh_protect_memory` | `address, size, protection, method?` | Change page protection inside the target and return the previous protection. Supports `api` (default), `nt`, and `syscall`; an unavailable syscall falls back to nt |
...
| `veh_batch` | `steps, inputs?, input_variable?, stop_on_error?` | Execute commands and repeat them sequentially per input in one session, with references/control flow plus per-input status, counts, first failure, trace summaries, and artifact metadata. |
...
| `veh_targeted_capture` | `inputs, steps?, trace, trigger, window, environment?, output_directory, stop_on_error?` | Apply per-input setup and write occurrence-centered ordered trace/environment artifacts; only nonzero traces completed by the requested target-window stop count as successful. |
| `veh_checkpoint_create` | `threadId, regions?, capture_teb?, teb_size?` | Capture GPR/flags (plus x64 XMM), TEB and FS/GS environment, and selected memory. Optional TEB bytes are comparison-only OS-managed state; stack regions identify the safe logical restore range from saved SP upward. |
| `veh_checkpoint_restore` | `id` | Restore context and selected memory while the original thread is VEH-stopped; preserve live VEH stack frames below saved SP, refuse changed mappings, and attempt rollback on failure. |
...
> **Non-stop inspection (no target stop required)**: `veh_read_memory` / `veh_read_pointer_chain` / `veh_write_memory` / `veh_dump_memory` / `veh_disassemble` / `veh_modules` / `veh_memory_map` / `veh_protect_memory` / `veh_search_memory` / `veh_value_scan` / `veh_symbolize` / `veh_display_type` work while the target is **running** (serviced by a dedicated pipe thread inside the DLL -- other threads are never frozen). You don't need a breakpoint or a detach/attach round-trip to read live values during GUI interaction. In contrast, `veh_registers` / `veh_stack_trace` / `veh_enum_locals` / `veh_step_*` need a thread context, so they only work when stopped at a breakpoint or after `veh_pause`.
