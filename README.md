...
- **MCP 도구 서버**: AI 에이전트(Claude, Codex 등)가 직접 디버거를 제어하는 34개 도구 제공
...
| `veh-mcp-server.exe` | MCP 도구 서버. AI 에이전트가 34개 도구로 디버거 직접 제어 |
...
| `veh_resolve_imports` | `threadId, addresses, max_steps?` | 난독화 import 일괄 해석 (thunk -> DLL 스텝 추적, 최대 2000개) |
