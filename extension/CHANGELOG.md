...
#### Critical
- **Reader thread deadlock on conditional breakpoints** — `EvaluateCondition`/`ExpandLogMessage` called `SendAndReceive` from the IPC reader thread callback, blocking the only thread that could read the response → timeout/hang. Fixed by embedding `RegisterSet` in `BreakpointHitEvent` payload and using `ReadProcessMemory` (direct Win32) for memory reads
- **Memory read values corrupted** — All `ReadMemory` IPC responses include a 4-byte `IpcStatus` header that was not skipped. Hover preview, conditional breakpoints, log points, and `readMemory` DAP request all returned data offset by 4 bytes
