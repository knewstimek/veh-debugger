...
json McpServer::ToolTraceBasicBlocks(const json& args) {
	...
	auto parseArgAddress = [&](const char* name, uint64_t& value) -> bool {
		if (!args.contains(name)) return false;
		const auto& item = args[name];
		if (item.is_string()) return ParseAddress(item.get<std::string>(), value);
		if (item.is_number_unsigned() || item.is_number_integer()) {
			value = item.get<uint64_t>(); return true;
		}
		return false;
	};
	uint64_t start = 0, end = 0;
	if (!parseArgAddress("start", start) || !parseArgAddress("end", end) || start >= end)
	...
	int maxBlocks = JsonInt(args, "max_blocks", 4096);
	int maxEdges = JsonInt(args, "max_edges", 8192);
	int maxSteps = JsonInt(args, "max_steps", 100000);
	int timeoutMs = JsonInt(args, "timeout_ms", 10000);
	int stackBytes = JsonInt(args, "stack_bytes", 128);
	...
	bool followExceptions = args.value("follow_exceptions", true);

	auto result = session_.TraceBasicBlocks(threadId, start, end,
	...
	if (!result.ok) return {{"error", "TraceBasicBlocks failed (thread must be VEH-stopped and RIP must be inside the range)"}};
	...
	for (size_t index = 0; index < result.snapshots.size(); ++index) {
		...
		for (uint16_t i = 0; i < snapshot.stackSize; ++i) {
			if (i) stack << ' ';
			stack << std::hex << std::setfill('0') << std::setw(2) << static_cast<unsigned>(snapshot.stack[i]);
		}
